...
const FOAM: PackedScene = preload("res://Scenes/foam.tscn")
...
func check_grass_neigbors(cell: Vector2i) -> bool:
	...
	var up_neigbor: Vector2i = Vector2i(cell.x, cell.y - 1)
	var bottom_neigbor: Vector2i = Vector2i(cell.x, cell.y + 1)
	
	var neigbors_list: Array = [left_neigbor,right_neigbor, up_neigbor, bottom_neigbor]
	
	for neigbor in neigbors_list:
		if water_used_cells.has(neigbor):
			return true
	return false
